...
export default function SecurityExpert() {
  ...
  return (
    <section className="relative min-h-screen bg-[#020305] flex items-center justify-center overflow-hidden font-mono px-4 py-12 md:py-0">
      
      {/* 1. BACKGROUND TECH-GRID */}
      <div className="absolute inset-0 z-0 opacity-10 md:opacity-20" 
           style={{ backgroundImage: `radial-gradient(#d4af37 0.5px, transparent 0.5px)`, backgroundSize: '20px 20px md:30px 30px' }} />

      <AnimatePresence mode="wait">
        {!isUnlocked ? (
          /* STATE 1: THE LOCK (The Hook) - Mobile Optimized */
          <motion.div 
            key="locked"
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
          ...
          /* STATE 2: THE REVEAL (The Epic Intro) - Mobile Optimized */
          <motion.div 
            key="unlocked"
          ...
          >
            ...
            <div className="border border-[#D4AF37]/20 bg-black/60 backdrop-blur-xl p-4 sm:p-6 md:p-8 rounded-sm relative overflow-hidden group">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0 sm:items-start mb-6 md:mb-8">
                ...
                <div className="text-left sm:text-right text-[8px] sm:text-[9px] text-white/30 uppercase leading-tight font-mono">
                  Subject: WAKESHO_01<br/>Status: Field_Active<br/>Clearance: Level_IV
                </div>
              </div>

              <div className="mb-6 md:mb-8">
                <p className="text-[8px] sm:text-[9px] md:text-[10px] text-[#D4AF37] font-mono uppercase tracking-[0.2em] sm:tracking-[0.3em] mb-2">Professional Field</p>
                <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-serif text-white leading-tight">
                  Cyber <span className="text-[#D4AF37] italic">Architect</span>
                </h2>
              </div>

              <p className="text-white/40 sm:text-white/50 text-xs sm:text-sm leading-relaxed mb-6 md:mb-8 border-l border-[#D4AF37] pl-3 sm:pl-4">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 md:gap-4">
                <div className="px-2 sm:px-3 py-1 bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#D4AF37] text-[8px] sm:text-[9px] font-mono">CORE_COMPETENCY</div>
                <div className="px-2 sm:px-3 py-1 bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#D4AF37] text-[8px] sm:text-[9px] font-mono">SECURITY_EXPERT</div>
              </div>
            </div>

            {/* RIGHT: Live Feed / Skills - Mobile Optimized */}
            <div className="grid grid-cols-1 gap-3 sm:gap-4">
              {[
                ...
                <motion.div 
                  ...
                  key={i} 
                  className="flex items-center justify-between p-3 sm:p-4 md:p-6 bg-white/5 border border-white/10 hover:bg-white/10 transition-colors"
                >
                  <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                    <span className="text-[#D4AF37]">{skill.icon}</span>
                    <span className="text-white text-[10px] sm:text-[11px] md:text-xs uppercase tracking-widest">{skill.label}</span>
                  </div>
                  <span className="text-[#D4AF37] font-mono text-[10px] sm:text-[11px] md:text-xs">{skill.val}</span>
                </motion.div>
              ))}

              {/* Terminal Output - Mobile Optimized */}
              <div className="mt-2 sm:mt-3 md:mt-4 p-3 sm:p-4 bg-black border border-white/5 font-mono text-[8px] sm:text-[9px] md:text-[10px] text-green-500/60 leading-relaxed">
